...
# CCDRS ManagerTool
- The screenshots below describe how to use the CCDRSManager  

# Developer Notes
With respect to CCDRS, developers in a development environment need to disable 
the login as we don't have mongodb setup in the local environment. 
- Open program.cs and then comment out lines 46-55 the following 
lines of code below need to be commented out to remove the authentication 
stuff. This is because the authentication currently is using a mongodb.
- If you wish not to disable this then you need to set up a mongdb and locally 
connect to that. It is better to just disable this 
Look for the builder.services section and disable the code inside   
```
// Block pages and ensure all pages require authentication only for development. 
builder.Services.AddRazorPages(options => 
    { 
        options.Conventions.AuthorizePage("/Index"); 
        options.Conventions.AuthorizePage("/AllStation"); 
